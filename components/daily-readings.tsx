      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            <h2 className="text-xl font-bold">{format(selectedDate, "EEEE, MMMM d, yyyy")}</h2>
            <p className={`text-sm mt-1 ${seasonColorText} font-medium`}>{readings.liturgicalDay}</p>
          </div>
...
function ReadingItem({
  ...
  return (
    <div className={`border rounded-lg overflow-hidden ${isActive ? `border-2 ${colorBorder}` : "border-gray-200"}`}>
      ...
      {isActive && <div className="p-4 border-t border-gray-200 whitespace-pre-line">{reading.text}</div>}
    </div>
  )
}
